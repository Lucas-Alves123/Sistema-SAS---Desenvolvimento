...
export function tryPopulateHeaderUser() {
    ...
    const storedName = localStorage.getItem('sga_username');
    console.debug('[global] tryPopulateHeaderUser - storedName:', storedName, 'userNameElement?', !!userNameElement);
    if (!storedName) {
        redirectToLogin();
        return;
    }
    if (userNameElement) userNameElement.textContent = formatDisplayName(storedName);
    // populate avatar initial when present
...
export function initHeaderUserCommon() {
    ...
    if (logout) {
        logout.addEventListener('click', function(e) {
            e.preventDefault();
                // Full logout - remove all stored auth info
                localStorage.removeItem('sga_username');
                localStorage.removeItem('sga_login_time');
